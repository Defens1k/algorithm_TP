...
class Deck {
 public:
    ...
    void push_front(int64_t input_data);
...
int64_t Deck::pop_back() {
    if (is_empty()) {
        return -1;
    }
    else {
        int64_t out_data = data[back_index];
        if (size() == 1 + free_cell()) {
            resize(MIN_SIZE);
            empty = true;
            return out_data;
        }
        back_index = (back_index + size() - 1) % size();
        if ((free_cell() * 20 > 19 * size()) && (size() > MIN_SIZE)) {
            resize(size() / 2);
        }
        return out_data;
...
void Deck::push_back(int64_t input_data) {
    if (is_empty()) {
        data[0] = input_data;
        front_index = 0;
        back_index = 0;
...
void Deck::push_front(int64_t input_data) {
    if (is_empty()) {
        data[0] = input_data;
        front_index = 0;
        back_index = 0;
    ...
    if (front_index == 0) {
        data[size() - 1] = input_data;
        front_index = size() - 1;
        empty = false;
        return;
    }
    else {
        data[front_index - 1] = input_data;
        front_index--;
        empty = false;
    }
}
...
bool Deck::resize(int64_t new_size) {
